    ...
    const formatTime = (secs:any) => {
      ...
      const handleResend = async(e:any) => {
        ...
        try {
          if(userDetails){
            ...
            const { email, token } = JSON.parse(userDetails);
            const response=await axios.post(`${API_URL}/api/auth/resendOtp`,{token:token})
      ...
      const handlesignUpOTP=async()=>{
        ...
        if(otp&&userDetails){
          try {
            const { email, token } = JSON.parse(userDetails);
            const response=await axios.post(`${API_URL}/api/auth/otpVerify`,{otp:otp,token:token})
            message.success(response.data.message)
            navigate("/login")
          } catch (error:any) {
              console.log(error);
                        ...
                        <div>
                            <div className='d-flex justify-content-between flex-column' style={{height:"60vh"}}>
                                ...
                                <div className='text-center pt-3'>
                                    <Button variant="contained" className='create-btn' onClick={handlesignUpOTP}>Done</Button>
                                </div>
                                <div>
                                        <Snackbar
                                            open={open}
